...
export async function registerAndLoginPlayer(app: INestApplication) {
  ...
  const registerResponse = await APIUtils.buildRequest(app, 'post', '/auth/register').send({
    username,
    email,
    password: 'password'
  })
  ...
  const loginResponse = await APIUtils.buildRequest(app, 'post', '/auth/login').send({
    username: username,
    password: 'password'
  })
