...
describe('user', () => {
  ...
  it('should not save user with invalid email', async () => {
    ...
    await expect(factory.create(user)).rejects.toThrowError(
      /Validation failed!/
    );
  });
  ...
  it('should not save user with the same email', async () => {
    ...
    await expect(factory.create(user2)).rejects.toThrowError(
      /UNIQUE constraint/
    );
  });
  ...
  it('should not save password in a readable form', async () => {
    ...
    const dbuser1 = await factory.create(user1);
    expect(dbuser1.password).not.toBe(user1.password);

    const user2 = factory.makeDummy();
    const dbuser2 = await factory.create(user2);
...
describe('comparePassword', () => {
  it('should return true for valid password', async () => {
    const user = factory.makeDummy();
    const dbuser = await factory.create(user);
  ...
  it('should return false for invalid password', async () => {
    const user = await factory.createDummy();
    expect(
      await userRepo.comparePassword(user.id, faker.internet.password())
    ).toBe(false);
  });
  ...
  it('should update password hash if password is updated', async () => {
    const user1 = factory.makeDummy();
    const dbuser1 = await factory.create(user1);
    expect(await userRepo.comparePassword(dbuser1.id, user1.password)).toBe(
      true
    );
    ...
    expect(await userRepo.comparePassword(dbUser2.id, user1.password)).toBe(
      false
    );
  });
});
