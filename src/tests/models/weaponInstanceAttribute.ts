...
const factory = new WeaponInstanceAttributeFactory();

beforeAll(async () => {
  await DB.init();
  repo = factory.getRepository();
});
describe('weaponInstanceAttribute', () => {
  ...
  it('should not save weaponInstanceAttribute without attribute', async () => {
    const ai = await factory.makeDummyWithAll();
    delete ai.attribute;
    await expect(factory.create(ai)).rejects.toThrowError(
      /weapon_instance_attribute.attributeId/
    );
  });
