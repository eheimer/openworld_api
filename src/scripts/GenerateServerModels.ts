...
const source = path.join(serverRoot, 'src')
const inputFile = path.join(serverRoot, 'config/openapi.yml')
const outputPath = path.join(source, 'api/dto')
const jsonFilePath = path.join(serverRoot, 'build')
...
for (const className in classMap) {
  ...
  const outFile = path.join(outputPath, classMap[className].path ? classMap[className].path : '', `${className}.ts`)
