...
export abstract class CreatureService {
   ...
   * @param {*} monsterid
   */
  static async generateCreature(monsterId: number | string): Promise<CreatureInstance> {
    //TODO change this call when we finally have a MonsterService
    const monster = await new MonsterFactory().getRepository().findOne({
      id: monsterId
    })
    const orighp = RandomService.getRandomInRange(monster.hp)
    const instance: CreatureInstance = await this.factory.create({
      monster,
      name: monster.name,
      orighp: orighp,
      hp: orighp,
