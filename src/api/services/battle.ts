...
async function createBattle(gameId: number | string, playerId: number | string): Promise<{battleId: number | string}> {
  try {
      const character = await new CharacterFactory().getRepository().findOne({ player: { id: playerId }, game: { id: gameId}})
      const battle = await factory.create({game: {id: gameId}, initiator: character, participants: [character]})
