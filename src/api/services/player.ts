...
async function updatePlayerLastSeen(
...
async function getPlayerWithGames(playerId: number | string): Promise<User> {
  try {
    return await factory
      .getRepository()
      .findOne(playerId, { relations: ['games', 'games.owner'] });
  } catch (err) {
...
async function getGameCharacters(
  playerId: number | string
  ...
  try {
    ...
    for (const game of games) {
      const char = await charFactory
        .getRepository()
        .findOne({ player: { id: playerId }, game: game });
...
async function getPlayerWithRelations(
  playerId: number | string
): Promise<User> {
  try {
    return await factory
