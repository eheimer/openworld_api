...
export async function getGames(
  ...
  try {
    ...
    const gameChars: GameCharacter[] = await PlayerService.getGameCharacters(
      playerId
    );
...
export async function createGame(
  ...
  if (res.locals.auth.userId) {
    try {
      const resp = await GameService.createGame(
        name,
        maxPlayers,
        res.locals.auth.userId
      );
...
export async function updateGame(
  ...
  const { gameId } = req.params;
  try {
    const game = await GameService.authorizeOwner(
      gameId,
      res.locals.auth.userId
    );
    if (!game) {
...
export async function leaveGame(
  ...
  try {
    ...
    if (
      (game as { error }).error === 'unauthorized' &&
      playerId !== res.locals.auth.userId
    ) {
      return respond.UNAUTHORIZED(res);
    }
...
export async function getGameBattles(
  req: express.Request,
  res: express.Response
): Promise<void> {
  const { gameId } = req.params;
