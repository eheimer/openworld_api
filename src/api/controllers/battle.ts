...
export async function createBattle(req: express.Request, res: express.Response): Promise<void> {
    ...
    try {
        let resp = await BattleService.createBattle((gameId as unknown as number), res.locals.auth.userId)
        let path = makeRoutePath('getBattle', { gameId, battleId: (resp as any).battleId })
